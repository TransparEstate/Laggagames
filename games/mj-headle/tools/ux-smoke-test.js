...
section('static UI markers');
{
  ...
  assert.ok(js.includes('mj-headle-volume-reveal'));
  assert.ok(js.includes('getPlayVolume') || js.includes('VOLUME_PLAY'));
  assert.ok(js.includes("applyVolume(el, 'play')") || js.includes("applyVolume(el, \"play\")"));
  assert.ok(js.includes("applyVolume(el, 'reveal')") || js.includes("applyVolume(el, \"reveal\")"));
