...
let DATA_FILE = process.env.FLAG_RUSH_HS_FILE
  ? path.resolve(process.env.FLAG_RUSH_HS_FILE)
  : DEFAULT_DATA_FILE;
const TOP_N = 10;
const MAX_NAME = 24;
...
function sanitizeName(name) {
  return String(name || 'Spieler').trim().slice(0, MAX_NAME) || 'Spieler';
}
...
function recordRaceFinish(room) {
  if (!room || room.phase !== 'finished') return { ok: false, skipped: true };
  if (room.raceHighscoreRecorded) return { ok: true, already: true, results: [] };
  const rounds = room.totalRounds || room.settings?.rounds || 10;
